...
func _process(_delta: float) -> void:
	...
	if race_manager.current_track_card.first_link == race_manager.current_track_card.circuit_link_1:
		...
		if (path_follow_2d.progress_ratio < 1 - race_manager.current_track_card.speed_min_limit_1 and path_follow_2d.progress_ratio > 1 - race_manager.current_track_card.speed_max_limit_1) or (path_follow_2d.progress_ratio < 1 - race_manager.current_track_card.speed_min_limit_2 and path_follow_2d.progress_ratio > 1 - race_manager.current_track_card.speed_max_limit_2):
			if speed >= race_manager.current_track_card.speed_threshold:
				print("SPIN!")
		elif (path_follow_2d.progress_ratio < 1 - race_manager.current_track_card.caution_min_limit_1 and path_follow_2d.progress_ratio > 1 - race_manager.current_track_card.caution_max_limit_1) or (path_follow_2d.progress_ratio < 1 - race_manager.current_track_card.caution_min_limit_2 and path_follow_2d.progress_ratio > 1 - race_manager.current_track_card.caution_max_limit_2):
			if speed >= race_manager.current_track_card.speed_threshold:
				print("Caution")
